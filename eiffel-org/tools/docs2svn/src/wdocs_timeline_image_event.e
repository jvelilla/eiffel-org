note
	description: "Summary description for {WDOCS_TIMELINE_IMAGE_EVENT}."
	author: ""
	date: "$Date: 2014-12-03 12:12:08 -0300 (mi. 03 de dic. de 2014) $"
	revision: "$Revision: 96232 $"

class
	WDOCS_TIMELINE_IMAGE_EVENT
...
feature {NONE} -- Initialization
	...
	make (a_base_dir: PATH; a_image_key: READABLE_STRING_GENERAL; a_image_path: PATH; a_book_id: READABLE_STRING_GENERAL; a_rev: WDOCS_REVISION_INFO; a_date: DATE_TIME)
		do
			base_dir := a_base_dir
			location := a_image_path
			name := a_image_key
	...
	relative_path: STRING_32
		...
		do
			...
			if book.is_whitespace then
				if Result.starts_with ("_images") then
					-- Keep it as it is
				else
					i := Result.substring_index ("_images", 1)
					if i > 0 then
						Result.remove_head (i - 1)  -- remove until end of "^(.*)_images"
					else
							-- should not occur
					end
				end
